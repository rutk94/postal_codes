...
def test_solve(setup_solver: Solver) -> None:
    solution = setup_solver.solve()
    assert solution is not None
    assert solution == setup_solver.solution


def test_solver_raises_no_solution_error(setup_solver: Solver, monkeypatch) -> None:
    mock_status = MagicMock(return_value=0)
    monkeypatch.setattr('ortools.constraint_solver.pywrapcp.RoutingModel.status', mock_status)
